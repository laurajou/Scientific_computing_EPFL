...
DoubleMatrix2x2 DoubleMatrix2x2::Inverse() const {
  ...
  if (det == 0) {
    std::cout << "The determinant is 0. Returning an zero matrix" << std::endl;
    return DoubleMatrix2x2();
  } else {
    return DoubleMatrix2x2(Get(1, 1) / det, -Get(0, 1) / det, -Get(1, 0) / det,
                           Get(0, 0) / det);
  }
}
...
DoubleMatrix2x2 DoubleMatrix2x2::operator-() const {
  return DoubleMatrix2x2(-mMatrix[0], -mMatrix[1], -mMatrix[2], -mMatrix[3]);
}

DoubleMatrix2x2 DoubleMatrix2x2::operator+(const DoubleMatrix2x2 &m) const {
  return DoubleMatrix2x2(mMatrix[0] + m.Get(0, 0), mMatrix[1] + m.Get(0, 1),
                         mMatrix[2] + m.Get(1, 0), mMatrix[3] + m.Get(1, 1));
}
